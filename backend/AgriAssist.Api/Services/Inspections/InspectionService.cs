...
{
    public async Task<PagedResult<FieldInspectionResponse>> SearchInspectionsAsync(PagedQuery query, Guid? fieldId, CancellationToken cancellationToken)
    {
        query.Normalize();
        ...
        inspections = query.SortDirection == "desc" ? inspections.OrderByDescending(item => item.ScheduledAt) : inspections.OrderBy(item => item.ScheduledAt);
    ...
    {
        query.Normalize();
        var observations = dbContext.InspectionObservations.AsNoTracking().Where(item => !item.IsDeleted);
    ...
    public async Task<PagedResult<CropIssueResponse>> SearchIssuesAsync(PagedQuery query, CancellationToken cancellationToken)
    {
        query.Normalize();
        ...
        issues = query.SortDirection == "desc" ? issues.OrderByDescending(item => item.CreatedAt) : issues.OrderBy(item => item.CreatedAt);
    ...
    public async Task<FollowUpRecommendationResponse> CreateRecommendationAsync(FollowUpRecommendationRequest request, CancellationToken cancellationToken)
    {
        ...
        await dbContext.SaveChangesAsync(cancellationToken);
        return new FollowUpRecommendationResponse(recommendation.Id, recommendation.CropIssueId, recommendation.Recommendation, recommendation.DueAt, recommendation.IsCompleted);
    }
    ...
    public async Task<InspectionImageResponse> UploadImageAsync(Guid inspectionId, IFormFile file, CancellationToken cancellationToken)
    {
        ...
        await dbContext.SaveChangesAsync(cancellationToken);
        return new InspectionImageResponse(image.Id, image.FieldInspectionId, image.Url, image.PublicId, image.ContentType, image.SizeBytes);
    }

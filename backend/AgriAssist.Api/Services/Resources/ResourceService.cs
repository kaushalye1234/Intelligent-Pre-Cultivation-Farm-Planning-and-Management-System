    ...
    IRequestValidator<ResourceReservationRequest> reservationValidator) : IResourceService
{
    ...
    public async Task<ResourceReservationResponse> ReserveAsync(ResourceReservationRequest request, CancellationToken cancellationToken)
    {
        ...
        if (stock.ReservedQuantity > stock.QuantityOnHand) throw new ApiException(HttpStatusCode.Conflict, "NEGATIVE_AVAILABLE_STOCK", "Reservation would make available stock negative.");
    ...
    public async Task<ResourceReservationResponse> ReleaseAsync(Guid reservationId, CancellationToken cancellationToken)
    {
        await using var transaction = await BeginTransactionIfRelationalAsync(cancellationToken);
        ...
        if (reservation.Status != ResourceReservationStatus.Active) throw new ApiException(HttpStatusCode.Conflict, "RESERVATION_NOT_ACTIVE", "Only active reservations can be released.");
        ...
        dbContext.StockTransactions.Add(new StockTransaction { InventoryStockId = stock.Id, Type = StockTransactionType.Release, Quantity = reservation.Quantity, Note = "Reservation released.", CreatedByUserId = currentUser.UserId });
        await dbContext.SaveChangesAsync(cancellationToken);
        if (transaction is not null) await transaction.CommitAsync(cancellationToken);
        return MapReservation(reservation);
    }
